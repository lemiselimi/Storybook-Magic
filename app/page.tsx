...
export default function StorybookCreator() {
  ...
  const getFallbackStory = () => ({
    ...
    pages: [
      { pageNum: 1, text: `Once upon a time, ${childName || "our hero"} woke up to find a magical map under their pillow.`, illustration: `${childName} sitting in bed holding a glowing treasure map` },
      { pageNum: 2, text: `${childName || "Our hero"} packed a backpack and set off. "I'm ready!" they cheered.`, illustration: `${childName} in explorer gear with backpack` },
  ...
  const generateBook = async () => {
    ...
    try {
      ...
      setStory(storyRes.status === "fulfilled" ? storyRes.value : getFallbackStory());
      setCurrentPage(-1);
      setStep("book");
    } catch (err) {
      console.error(err);
        ...
        {isLeft ? (
          <>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ width: "100%", aspectRatio: "4/3", borderRadius: 16, overflow: "hidden", background: bg, border: "3px solid rgba(255,255,255,0.5)", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
                {displayPhoto && (
                  <img src={displayPhoto} alt="hero" style={{
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #1a0a2e 0%, #2d1b4e 40%, #1a3a2e 100%)", fontFamily: "'Segoe UI', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "32px 16px" }}>
      ...
      {step === "generating" && (
        <div style={{ textAlign: "center", animation: "fadeUp 0.5s ease both", maxWidth: 400 }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13, margin: "0 0 28px" }}>Hang tight — about 30 seconds!</p>
