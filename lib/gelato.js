...
// Interior PDF is padded to an EVEN page count by generate-book-pdf (Gelato
// rejects odd counts and anything below the product minimum as "Page count is
// invalid"). The actual count is stored on result.interiorPageCount and used
// when present; this is only a fallback. Overridable via env.
const rawPages = Number(process.env.GELATO_INTERIOR_PAGES) || 32;
const INTERIOR_PAGE_COUNT = rawPages % 2 === 0 ? rawPages : rawPages + 1;
...
export async function submitPrintFromKV(ref, { dryRun = false } = {}) {
  ...
  // Gelato is the legacy provider (Prodigi is current). Its shipmentMethodUid
  // below is hardcoded to "normal", and no Gelato expedited UID has been
  // verified — so an Express order must NOT be fulfilled here. Refuse rather
  // than take Express money and ship standard.
  const paidShipping = shippingMethodFromStripeSession(session, process.env);
  if (paidShipping === "express") {
    throw new Error(
      "Express shipping was paid but the active print provider is Gelato, which has no verified expedited service. " +
      "Set PRINT_PROVIDER=prodigi to fulfil Express orders.",
    );
  }
  ...
  const res = await fetch(GELATO_API_URL, {
    ...
    body: JSON.stringify({
      ...
      // Standard only — see the Express guard above.
...
export function sendPrintFailureAlert({ ref, sessionId, error, context }) {
  ...
  fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html:    `<p><strong>Error:</strong> ${error}</p><p><strong>Ref:</strong> ${ref}</p><p><strong>Session:</strong> ${sessionId || "unknown"}</p><p>Use the protected operations workflow to retry this order.</p>`,
    }),
  }).catch(() => {});
}
